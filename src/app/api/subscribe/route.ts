...
async function checkRateLimit(ip: string) {
  ...
  return ratelimit.limit(`subscribe:${ip}`);
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (exists) {
      return NextResponse.json({ message: "Accès accordé" });
    }
    ...
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          subject: `Nouvel abonné guide En Clair (#${totalSubscribers})`,
          html: `<div style="font-family:Georgia,serif;max-width:500px;margin:0 auto;padding:32px;">
            <h2 style="color:#1a1a1a;margin:0 0 16px;">Nouvel abonné (guide)</h2>
